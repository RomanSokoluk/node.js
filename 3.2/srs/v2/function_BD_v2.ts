...
async function getBoks2(were?: { type: "year", year: string } | { type: "author", author: string } |
...
function deletBook2(book: { id: string }) {
  const get1 = `UPDATE table_books_2
SET delet=true
WHERE id=?;`
  return connection.query(get1, [book.id])
}
function notDeletBook2(book: { id: string }) {
  const get1 = `UPDATE table_books_2
SET delet=false
WHERE id=?;`
  return connection.query(get1, [book.id])
}
export { getBoks2, getBook2, chengeBook2, addBook2, deletBook2, notDeletBook2}